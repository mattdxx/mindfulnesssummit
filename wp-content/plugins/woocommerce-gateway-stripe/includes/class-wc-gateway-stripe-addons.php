	...
	public function process_subscription( $order_id, $retry = true ) {
		...
		try {
			...
			if ( $initial_payment > 0 ) {
				$payment_response = $this->process_subscription_payment( $order, $initial_payment, true );

				if ( is_wp_error( $payment_response ) ) {
					throw new Exception( $payment_response->get_error_message() );
				} elseif ( $payment_response->captured ) {
					$order->payment_complete( $payment_response->id );
					WC_Subscriptions_Manager::activate_subscriptions_for_order( $order );
				} else {
					$order->update_status( 'on-hold', sprintf( __( 'Stripe charge authorized. Process order to take payment, or cancel to remove the pre-authorization.', 'woocommerce-gateway-stripe' ), $payment_response->id ) );

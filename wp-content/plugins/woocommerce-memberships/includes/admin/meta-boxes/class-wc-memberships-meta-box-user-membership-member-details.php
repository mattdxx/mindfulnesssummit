...
class WC_Memberships_Meta_Box_User_Membership_Member_Details extends WC_Memberships_Meta_Box {
	...
	public function get_title() {
		return __( 'Member Details', WC_Memberships::TEXT_DOMAIN );
	}
	...
	public function output( WP_Post $post ) {
		...
		<p>
			<a href="mailto:<?php echo esc_attr( $user->user_email ); ?>" class="member-email"><?php echo esc_html( $user->user_email ); ?></a><br />
			<?php if ( $member_since ) : ?>
				<span class="member-since"><?php printf( esc_html__( 'Member since %s', WC_Memberships::TEXT_DOMAIN ), date_i18n( get_option( 'date_format' ), $member_since ) ); ?></span>
			<?php endif; ?>
		</p>
